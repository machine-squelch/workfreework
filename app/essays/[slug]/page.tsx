  ...
  return (
    <div className="bg-gray-800 min-h-screen">
      ...
      <article className="max-w-4xl mx-auto px-4 py-16">
        <div className="paper-sheet p-6 md:p-10 border-4 border-white neon-border neon-backglow--white outline outline-1 outline-black">
          <div
            style={{
              fontSize: '1.125rem',
              lineHeight: '1.75',
          ...
          >
            <div dangerouslySetInnerHTML={{ __html: sanitizeHTML(essay.content) }} />
          </div>
        </div>
      </article>
          ...
          {Object.entries(essaysData)
            ...
            .map(([slug, relatedEssay]) => (
              <Link key={slug} href={`/essays/${slug}`} className="group">
                <div className="bg-gray-900 p-6 rounded-lg hover:shadow-lg transition-all border border-gray-700 hover:border-white hover:shadow-[0_0_25px_rgba(255,255,255,0.4)]">
                  <h3 className="text-lg font-bold mb-2 text-white transition-opacity group-hover:opacity-90">
                    {relatedEssay.title}
                  </h3>
                  <p className="text-gray-400 text-sm">{relatedEssay.excerpt}</p>
                </div>
              </Link>
            ))}
        </div>
